...
Note 1 to entry: The concept of operations frequently is embodied in long‐range strategic plans and annual operational plans.
...
Note 1 to entry: An identified part of an organization (even as small as a single individual) or an identified group of organizations can be regarded as an organization if it has responsibilities, authorities and relationships.
...
Note 1 to entry: There are four agreed generic product categories: hardware (e.g., engine mechanical part); software (e.g., computer program procedures, and possibly associated documentation and data); services (e.g., transport); and processed materials (e.g., lubricant).
...
Note 2 to entry: In practice, the interpretation of its meaning is frequently clarified by the use of an associative noun, e.g., aircraft system or database management system.
...
Note 1 to entry: A system is able to accomplish its intended use, goals and objectives (i.e., meet stakeholder requirements) in the intended operational environment.
...
Note 1 to entry: Verification is a set of activities that compares a system or system element against the required characteristics.
